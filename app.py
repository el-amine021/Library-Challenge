...
@st.cache_data
def load_data():
    full_df = pd.read_csv("./DATASETS/BOOKS_DATASET_final_with_scores.csv")
    final_df = pd.read_csv("./DATASETS/selection_books.csv")
...
with tab1:
    ...
    st.markdown("### 📈 Score Final vs Publication year")
    ...
    plt.title("Final score vs Publication year", fontsize=16, fontweight='bold', color="darkblue")
    plt.xlabel("Publication year", fontsize=12, fontweight='bold')
    plt.ylabel("Final score", fontsize=12, fontweight='bold')

    plt.legend(title="Age group", fontsize=12, title_fontsize='13', loc='upper left', frameon=True, shadow=True, borderpad=1)
    ...
    st.download_button("📥 Download Full Dataset (CSV)", data=full_df.to_csv(index=False).encode("utf-8"), file_name="./DATASETS/BOOKS_DATASET.csv")
...
with tab2:
    ...
    st.download_button("📥 Download Final Book Selection (CSV)", data=final_df.to_csv(index=False).encode("utf-8"), file_name="./DATASETS/selection_books.csv")
